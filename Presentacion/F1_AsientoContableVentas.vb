...
Imports System.Math
Public Class F1_AsientoContableVentas
...
#Region "METODOS PRIVADOS"
    ...
    Private Sub _IniciarTodo()
        ...
        Me.Text = "COMPROBANTES DE VENTAS"
    ...
    Public Sub _prArmarCuadre(ByRef dt As DataTable)
        Try
            ...
            If (restantedebe > 0 Or restanteHaber > 0 Or RestanteDebeSus > 0 Or RestanteHaberSus > 0) Then
                Dim dtObtenerCuenta As DataTable = L_prCuentaDiferencia(244)  '''3=Lavadero
    ...
    Private Sub _prCargarTablaComprobantes()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            If (tipo = 1 Or tipo = 0) Then ''''Venta Contado
            ...
            If (total > 0) Then
                ...
                Else
                    ...
                    If (tipo = 1) Then  ''' Contado
                        dtCategorias = L_prObtenerCategoriasContado(2, 1, tbFechaI.Value.ToString("yyyy/MM/dd"), tbFechaF.Value.ToString("yyyy/MM/dd"))
                    End If
                    If (tipo = 0) Then  ''' Credito
                        dtCategorias = L_prObtenerCategoriasCredito(6, 1, tbFechaI.Value.ToString("yyyy/MM/dd"), tbFechaF.Value.ToString("yyyy/MM/dd"))
                    End If


                    For j As Integer = 0 To dtCategorias.Rows.Count - 1 Step 1

                        numiCuenta = dtCategorias.Rows(j).Item("canumi")
                        Dim dtObtenerCuenta As DataTable = L_prCuentaContable(numiCuenta)
                        tabla.Rows.Add(dtObtenerCuenta.Rows(0).Item("canumi"), dtObtenerCuenta.Rows(0).Item("cacta"), dtObtenerCuenta.Rows(0).Item("cadesc"), DBNull.Value, DBNull.Value, DBNull.Value, DBNull.Value, DBNull.Value, DBNull.Value, DBNull.Value, DBNull.Value, Escuela, 0) '''' Aqui agrego al padre
                        Dim dtTotalCategoria As DataTable
                        If (tipo = 1) Then
                            dtTotalCategoria = L_prObtenerTotalCategoriasContado(2, 1, tbFechaI.Value.ToString("yyyy/MM/dd"), tbFechaF.Value.ToString("yyyy/MM/dd"), dtCategorias.Rows(j).Item("CategoriaId"))

                        Else
                            dtTotalCategoria = L_prObtenerTotalCategoriasCredito(6, 1, tbFechaI.Value.ToString("yyyy/MM/dd"), tbFechaF.Value.ToString("yyyy/MM/dd"), dtCategorias.Rows(j).Item("CategoriaId"))
                        End If

                        If (dtTotalCategoria.Rows.Count > 0) Then
                            Dim Glosa02 As String = dtCategorias.Rows(j).Item("cadesc")
                            Dim conversion02 As Double = (dtTotalCategoria.Rows(0).Item("total") * (porcentaje / 100))
                            conversion02 = to3Decimales(conversion02)
                            Dim totales02 As Double = Round(conversion02, 2)
                            Dim TotalSus02 As Double = Round(to3Decimales(totales02 / (tbTipoCambio.Value)), 2)

                            If (dt.Rows(i).Item("chdebe") > 0) Then
                                tabla.Rows.Add(numiCuenta, DBNull.Value, Glosa02 + " DEL " + tbFechaI.Value.ToString("dd/MM/yyyy") + " AL " + tbFechaF.Value.ToString("dd/MM/yyyy"), DBNull.Value, DBNull.Value, DBNull.Value, tbTipoCambio.Value, totales02, DBNull.Value, TotalSus02, DBNull.Value, Escuela, Linea)
                            Else
                                tabla.Rows.Add(numiCuenta, DBNull.Value, Glosa02 + " DEL " + tbFechaI.Value.ToString("dd/MM/yyyy") + " AL " + tbFechaF.Value.ToString("dd/MM/yyyy"), DBNull.Value, DBNull.Value, DBNull.Value, tbTipoCambio.Value, DBNull.Value, totales02, DBNull.Value, TotalSus02, Escuela, Linea)
                            End If
                        End If
    ...
    Private Sub _prGuardarModificado()
        ...
        If res Then
            ...
            tbNumi.Text = Str(numiComprobante)
    ...
    Private Sub _prImprimir()
        ...
        objrep.SetParameterValue("titulo2", "MATNOVO S.R.L. " + gs_empresaDesc.ToUpper)
    ...
    Private Sub _prImprimirComprobante(numiComprobante)
        ...
        objrep.SetParameterValue("titulo", "MATNOVO S.R.L. " + gs_empresaDesc.ToUpper)
        objrep.SetParameterValue("nit", gs_empresaNit.ToUpper)
